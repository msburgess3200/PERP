...
function GM.ShowVIPDealershipView ( )		
	local W, H = 500, 250;
	
	local DermaPanel = vgui.Create("DFrame")
	DermaPanel:SetPos(ScrW() * .5 - W * .5, ScrH() * .5 - H * .5)
	DermaPanel:SetSize(W, H)
	DermaPanel:SetTitle("VIP Vehicle Dealership - Bank Balance: " .. DollarSign() .. LocalPlayer():GetBank() .. " - Current Sales Tax: " .. GAMEMODE.GetTaxRate_Sales_Text())
	DermaPanel:SetVisible(true)
	DermaPanel:SetDraggable(false)
	DermaPanel:ShowCloseButton(true)
	DermaPanel:SetAlpha(GAMEMODE.GetGUIAlpha());
	...
	for k, v in pairs(VEHICLE_DISPLAY_ORDER_VIP) do
		local table = VEHICLE_DATABASE[v];
