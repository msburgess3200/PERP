...
if (SERVER) then
	...
	TO_KILL:Kill()
				...
				//SendLog(PLAYER:Nick(), "KILL_PLAYER", TO_KILL:Nick(), nil) <- Broken, not defined/nil value.
	...
	concommand.Add("ASS_SlayPlayer", PLUGIN.KillPlayer)
...
if (CLIENT) then
	...
	function PLUGIN.KillPlayer(PLAYER)
		...
		RunConsoleCommand( "ASS_SlayPlayer", PLAYER:UniqueID() )
